...
class ThreadedTCPRequestHandler(SocketServer.StreamRequestHandler):
	...
	def __receiveFile__(self):
		...
		#if(locked_currDir!=DEFAULT_CURR_DIR):
		#	shutil.copy(fileName, locked_currDir)#overwrites same file if it exists
		#	time.sleep(1)
		#	os.remove(fileName)
	...
	def __runCommand__(self):
		...
		if len(commandArgs)==0:
			...
			proc = subprocess.Popen([command, commandArgs], stdout=subprocess.PIPE, shell=True)
			(out, err) = proc.communicate()
...
def getList(targ):
	...
	for i in range(len(fList)-1,-1,-1):
		...
		if targ=="file":
			if(os.path.isfile(fList[i])==False):
				fList.remove(fList[i])
		
		elif targ=="dir":
			if(os.path.isdir(fList[i])==False):
				fList.remove(fList[i])
